...
namespace BedAndBreakfastGroup16.Areas.Identity.Pages.Account
{
    public class RegisterModel : PageModel
    {
        ...
        private readonly IUserStore<BedAndBreakfastGroup16User> _userStore;
        ...
        private BedAndBreakfastGroup16User CreateUser()
        {
            try
            {
                return Activator.CreateInstance<BedAndBreakfastGroup16User>();
            }
            catch
            {
                throw new InvalidOperationException($"Can't create an instance of '{nameof(BedAndBreakfastGroup16User)}'. " +
                    $"Ensure that '{nameof(BedAndBreakfastGroup16User)}' is not an abstract class and has a parameterless constructor, or alternatively " +
                    $"override the register page in /Areas/Identity/Pages/Account/Register.cshtml");
            }
        }

        private IUserEmailStore<BedAndBreakfastGroup16User> GetEmailStore()
        {
            if (!_userManager.SupportsUserEmail)
            {
                throw new NotSupportedException("The default UI requires a user store with email support.");
            }
            return (IUserEmailStore<BedAndBreakfastGroup16User>)_userStore;
        }
    }
}
